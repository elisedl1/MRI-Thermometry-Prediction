...
def train_network(args, model,test_ids, train_ids, valid_ids,image_path,thermometry_path): # data_ds change to test_ids and train_ids, valid_ids


    # Collect Logger Info for model parame6ters
    logging.info('Starting train_network()...')
    logging.info('Energy Level: 500J - Epochs: {} - Learning Rate: {} - Batch Size: {} - Extra: Augmentation(2region)'.format(args.epochs, args.lr, args.batch_size))
    ...
    additional = "DataAug"
    torch.save(best_weights, output_model_path + f"/{args.epochs}_{args.lr}_{args.batch_size}_{test_ssim}_{additional}_.pth")
...
if __name__ == '__main__':
    ...
    if not test :  # training
        ...
        epoch_loss_list_ours = train_network(args, model, test_ids, train_ids, valid_ids,MRI_path,thermometry_path)
